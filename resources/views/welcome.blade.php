            ...
            <div
                ...
                <ul class="flex md:flex-row flex-col md:items-center md:gap-[1vw] gap-8">
                    ...
                    <li class="relative">
                        ...
                        <!-- Dropdown menu -->
                        <div id="newsDropdown" class="hidden absolute left-0 mt-1 w-28 bg-white rounded-md shadow-lg py-1 z-10">
                            <a href="{{ route('media') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Media</a>
                            <a href="{{ route('csr') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">CSR</a>
                        </div>
                ...
                @foreach ($featuredPosts as $post)
                    <div class="border p-4 rounded">
                        @if ($post->featured_image)
                            <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }}" class="mb-2">
                        @endif
                        <h2 class="text-xl font-semibold">{{ $post->title }}</h2>
                        <p class="text-gray-600 text-sm">{{ $post->created_at->format('d M, Y') }}</p>
                        <p class="whitespace-normal break-words">{{ \Illuminate\Support\Str::limit($post->content, 100) }}</p>
                    </div>
                @endforeach
            </div>
